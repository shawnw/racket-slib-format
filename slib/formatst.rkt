...
; Easier than changing probably hundreds of cases
...
(define fails 0)
(define total 0)
(define test-verbose #t)		; shows each test performed
...
(void (format #t "This implementation has~@[ no~] flonums ~
...
(parameterize ([format:symbol-case-conv string-downcase])
  (test '("~s" abc) "abc"))
(parameterize ([format:symbol-case-conv string-titlecase])
  (test '("~s" abc) "Abc"))
...
(parameterize ([format:iobj-case-conv string-titlecase])
  (test `("~s" ,display) (string-titlecase (format:iobj->str display #f))))
...
(void (format #t "~%~a Test~:p completed. (~a failure~:p)~2%" total fails))
