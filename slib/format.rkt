...
(require (only-in racket/base [if %primitive-if]))
(define-syntax if
  (syntax-rules (not)
    [(if (not test) body)
     (unless test body)]
...
(define-parameter format:symbol-case-conv #f)
...
(define (format . args)
  ...
  (define (format:out-fixed modifier number pars)
	...
	(if digits
	    ...
	    (begin			; fixed precision
	      (format:parse-float
	       (if (string? number) number (number->string (exact->inexact number))) #t scale)
  ...
  (define (format:out-expon modifier number pars)
	...
	(if digits			; fixed precision
	    ...
	    (let ((digits (if (> scale 0)
			      ...
			      digits)))
	      (format:parse-float
	       (if (string? number) number (number->string (exact->inexact number))) #f scale)
	      (if (<= (- format:fn-len format:fn-dot) digits)
		  (format:fn-zfill #f (- digits (- format:fn-len format:fn-dot)))
		  (format:fn-round digits))
	      (if width
  ...
  (define (format:out-general modifier number pars)
	...
	(format:parse-float
	 (if (string? number) number (number->string (exact->inexact number))) #t 0)
	(format:fn-strip)
  ...
  (let ((args (if (string? (car args)) (cons #f args) args)))
	...
	(let ((port (cond ((boolean? destination) (current-output-port))
	  ...
	  (set! format:port port)    ; global port for output routines
          (port-count-lines! port) ; Racket: Turn on position tracking
	  ...
	  (if format:flush-output (flush-output format:port))
	  #t))
